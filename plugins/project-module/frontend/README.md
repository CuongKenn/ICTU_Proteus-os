# Project Module — Frontend (Micro-UI thật)

Source of truth: `plugins/project-module/frontend/`
...
frontend/src/
  ...
  pages/*.tsx      → Dashboard, Projects, Tasks (kanban), Milestones
  App.tsx          → sub-router nhận { subPath, navigate }
```

## Data layer — 2 chế độ sau cùng 1 interface async (`src/lib/repo.ts`)

- **LIVE** (`createLiveStore`): đọc thật qua `GET /api/v1/plugins/project-module/records/*`
  (BFF proxy + session cookie, `credentials:"include"`), ghi CRUD qua records
  `POST/PATCH/DELETE`, riêng **Tạo task** dispatch workflow n8n
  `wf_task_assignment` qua `POST .../actions/wf_task_assignment`.
  Chuyển stage task = `PATCH status`, cột mốc = `PATCH completion_pct`.

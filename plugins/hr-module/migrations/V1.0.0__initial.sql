...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger WHERE tgname = 'set_hr_employees_updated_at'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger WHERE tgname = 'set_hr_leave_requests_updated_at'
    ) THEN

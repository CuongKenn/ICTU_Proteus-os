// Asset Module — root component. Gateway render <AssetModuleApp subPath navigate />.
// subPath: phần sau /asset-module ('' | 'assets' | 'maintenance' | 'disposals').
...
export function AssetModuleApp(props: { subPath: string; navigate: (sub: string) => void }) {
  const route = (props.subPath || '').replace(/^\/+|\/+$/g, '');
  const { mode } = useStore();
  return (
    <div style={{ padding: '1.5rem', maxWidth: 1200, margin: '0 auto' }}>
      <header style={{ marginBottom: '1rem' }}>
        <h2 style={{ margin: 0, color: '#e2e8f0' }}>{assetMeta.displayName}</h2>
        <p style={{ margin: '.25rem 0 0', color: '#94a3b8', fontSize: '.88rem' }}>{assetMeta.description}</p>
        <nav style={{ display: 'flex', gap: '.5rem', marginTop: '.75rem', flexWrap: 'wrap' }}>
          {assetMeta.routes.map((r) => {
      ...
      {route === 'disposals' && <DisposalsPage />}
      {!['', 'assets', 'maintenance', 'disposals'].includes(route) && (

...
- **Luồng xử lý:** Hệ thống tự động đồng bộ tài liệu từ Không gian [H] -> Băm nhỏ văn bản (Chunking) -> Chuyển hóa vector (Embedding) -> Lưu trữ tĩnh tại Qdrant.
...
- **Giới hạn:** Tác tử này chỉ **BÁO CÁO**, không được phép tự thực thi bất kỳ hành động nào vào dữ liệu.
...
- **Mô tả:** Hệ thống cốt lõi do đội tự code (Python/Node.js) để quản lý vòng đời của các Plugin. Bao gồm một giao diện Web "App Store" (Marketplace) để **Quản trị viên Tenant** (`tenant_admin`) cài đặt các module nghiệp vụ chỉ bằng một cú click chuột. Chỉ `tenant_admin` (Admin của tổ chức) và `superadmin` (ICTU Platform Team) mới có quyền này.
- **Luồng xử lý Cài đặt:** `tenant_admin` bấm nút "Cài đặt" trên giao diện Marketplace (hoặc lệnh `proteus-os install`) -> Hệ thống đọc `manifest.yaml` -> Tự động gọi API Postgres để tạo bảng -> Gọi API n8n để nạp luồng quy trình -> Gọi API Metabase để tạo Dashboard -> **Gọi API Appsmith để nạp Giao diện người dùng (UI Injection)**.

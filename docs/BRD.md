...
- **Luồng xử lý Cài đặt:** Quản trị viên bấm nút "Cài đặt" trên giao diện Marketplace (hoặc lệnh `proteus-os install`) -> Hệ thống đọc `manifest.yaml` -> Tự động gọi API Postgres để tạo bảng -> Gọi API n8n để nạp luồng quy trình -> Gọi API Metabase để tạo Dashboard -> **Gọi API Appsmith để nạp Giao diện người dùng (UI Injection)**.
...
- **Giao tiếp liên Plugin (Event Bus):** Các Plugin giao tiếp lỏng lẻo với nhau qua một Event Bus trung tâm (ví dụ Redis Pub/Sub hoặc RabbitMQ). Plugin A phát sự kiện (Publish), Plugin B lắng nghe (Subscribe) thông qua n8n Webhook, đảm bảo không bị "dính" logic (Decoupled).

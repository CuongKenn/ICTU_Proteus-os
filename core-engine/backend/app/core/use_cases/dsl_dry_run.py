...
class DSLDryRunEngine:
    ...
    async def execute_dry_run(self, tenant_id: str, dsl_payload: dict) -> Dict[str, Any]:
        """
        ...
        if effect not in ["write", "critical"]:
            return {"affected_count": 0, "preview": [], "message": "Dry run skipped for read effect."}
        ...
        try:
            ...
            sql_count = text(f"SELECT COUNT(*) FROM {target_table} WHERE tenant_id = :tenant_id AND status = 'pending'")
            ...
            # Lấy preview (limit 3)
            sql_preview = text(f"SELECT * FROM {target_table} WHERE tenant_id = :tenant_id AND status = 'pending' LIMIT 3")

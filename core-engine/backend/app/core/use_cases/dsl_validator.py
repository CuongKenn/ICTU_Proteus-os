...
class DSLValidator:
    ...
    async def validate(self, dsl_payload: Dict[str, Any]):
        ...
        if version != "1.0":
            raise DSLVersionCompatError(f"Unsupported DSL version: {version}. Expected 1.0.")

        # Parse action structure: e.g. "hr.leave_requests.batch_approve"
        action = dsl_payload.get("action", "")
        parts = action.split(".")
        if len(parts) < 3:
            raise DSLInvalidActionError(f"Invalid action format: {action}. Expected plugin.resource.method")
        ...
        whitelist_actions = ["hr.leave_requests.batch_approve", "hr.employees.get", "finance.invoices.create", "core.users.invite"]
        ...
        if plugin_code != "core":
            ...
            """)
            res = await self.db.execute(sql, {"tenant": self.tenant_id, "plugin": plugin_code})
            row = res.fetchone()
            if not row or row.status != 'ACTIVE':
                raise DSLPluginNotActiveError(f"Plugin {plugin_code} is not installed or not ACTIVE.")
        ...
        if not has_permission:
            raise DSLPermissionDeniedError(f"User {self.user_id} lacks permission {req_permission}")

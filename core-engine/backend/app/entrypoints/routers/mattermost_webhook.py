...
async def mattermost_interactive_callback(
    ...
    if action == "approve":
        ...
        cmd = await ai_command_use_case.ai_command_repo.get_command_by_id(uuid.UUID(action_id) if len(action_id) == 36 else uuid.UUID(int=0))
        success = await ai_command_use_case.process_approval(
            cmd_id=action_id, approver_id=user_id, action_taken="approve"
        )
        if success:
            actual_tenant_id = uuid.UUID(str(cmd["tenant_id"])) if cmd and cmd.get("tenant_id") else uuid.UUID(int=0)

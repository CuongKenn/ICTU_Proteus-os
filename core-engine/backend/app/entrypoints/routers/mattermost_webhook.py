...
async def mattermost_interactive_callback(
    ...
    if settings.MATTERMOST_WEBHOOK_SECRET:
        if not mattermost_signature or not verify_mattermost_signature(raw_body, mattermost_signature):
    ...
    if action == "approve":
        logger.info(f"Yêu cầu {action_id} được PHÊ DUYỆT bởi user {user_id}. Kích hoạt n8n execute.")

...
from app.adapters.repositories.base import AbstractTenantRepository, AbstractUserRepository
...
class OnboardingUseCase:
    ...
    async def execute(self, req: OnboardingRequest) -> dict[str, str]:
        logger.info("Bắt đầu xử lý đăng ký Tenant mới", extra={"email": req.admin_email, "company": req.company_name})
        ...
        try:
            ...
            keycloak_user_id = await self.keycloak_adapter.create_user(
                ...
                last_name=" ".join(req.admin_full_name.split(" ")[1:]) if " " in req.admin_full_name else "",
                attributes=user_attributes,
            )
        except ValueError as e:
            # User already exists
            logger.warning("Đăng ký thất bại: Email đã tồn tại", extra={"email": req.admin_email})
        ...
        logger.info("Đăng ký Tenant thành công", extra={"tenant_id": str(tenant_id), "keycloak_id": keycloak_user_id})

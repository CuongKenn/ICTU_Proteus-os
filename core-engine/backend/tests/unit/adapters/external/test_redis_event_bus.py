...
from app.adapters.external.redis_event_bus import RedisEventBusPublisher, EventBusPublishError
...
async def test_publish_success(adapter, mock_redis):
    with patch("app.adapters.external.redis_event_bus.aioredis.from_url") as mock_from_url:
        ...
        await adapter.publish("hr.employee.created", "tenant-1", "hr-module", {"name": "John"})
...
async def test_publish_plugin_lifecycle(adapter, mock_redis):
    with patch("app.adapters.external.redis_event_bus.aioredis.from_url") as mock_from_url:
        ...
        await adapter.publish_plugin_lifecycle("installed", "tenant-1", "hr-module", "1.0.0", extra_data={"status": "ok"})

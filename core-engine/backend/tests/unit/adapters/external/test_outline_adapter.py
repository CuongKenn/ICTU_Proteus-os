...
async def test_list_documents_success(adapter):
    mock_response = httpx.Response(200, json={"data": [{"id": "doc1", "urlId": "url1"}]}, request=httpx.Request("POST", "http://test"))
    
    mock_client_instance = AsyncMock()
    mock_client_instance.post.return_value = mock_response
    mock_client_instance.__aenter__.return_value = mock_client_instance
    
    with patch("app.adapters.external.outline_adapter.httpx.AsyncClient", return_value=mock_client_instance):
...
async def test_list_documents_http_error(adapter):
    ...
    mock_client_instance = AsyncMock()
    mock_client_instance.post.side_effect = httpx.HTTPStatusError("Error", request=mock_response.request, response=mock_response)
    mock_client_instance.__aenter__.return_value = mock_client_instance

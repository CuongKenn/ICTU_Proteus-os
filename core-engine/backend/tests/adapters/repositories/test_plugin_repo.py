...
async def test_list_marketplace(repo, mock_session):
    ...
    plugins, total = await repo.list_marketplace(limit=10, offset=0)
...
async def test_list_installed(repo, mock_session):
    ...
    plugins, total = await repo.list_installed(tenant_id)
    assert len(plugins) == 1
    assert total > 0
...
async def test_upsert_installation(repo, mock_session):
    ...
    await repo.upsert_installation(
        tenant_id, plugin_id, PluginStatus.INSTALLING, "1.0.0"
    )

...
async def test_plugin_install_e2e(async_db_engine, db_session):
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        ...
        with patch("app.adapters.external.n8n_adapter.N8nAdapter.import_workflow", new_callable=AsyncMock) as mock_import:
            mock_import.return_value = {"id": "1", "name": "Test Workflow", "active": True}

            # 2. Cài đặt hr-module
            headers = {"X-Tenant-ID": "tenant-e2e-1"}
            res_install = await ac.post("/api/plugins/hr-module/install", headers=headers)
            ...
            # 4. Uninstall Plugin
            res_uninstall = await ac.post("/api/plugins/hr-module/uninstall", headers=headers)

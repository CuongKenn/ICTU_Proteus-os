...
### Fixed
- **[core-engine/backend]** Sửa lỗi serialize không hợp lệ trong `AICommandUseCase` bằng cách sử dụng `json.dumps()` thay vì `str().replace("'", '"')`, đảm bảo dữ liệu lưu vào cột JSONB của bảng `ai_commands` là chuẩn JSON hợp lệ (Issue #276).

...
### Changed
- [core-engine/backend/app/core/use_cases/tenant_onboarding.py] Cải tiến API `get_integrations` lên "Level 2 Real Check": Ngoài việc kiểm tra biến môi trường (`.env`), Backend giờ đây sẽ sử dụng `httpx` + `asyncio.gather` để chủ động "ping" (gửi HTTP GET) đồng thời tới 5 dịch vụ lõi (Keycloak, Mattermost, Appsmith, n8n, Metabase). Nếu dịch vụ đang sập mạng (Network Down) hoặc time-out quá 1.5 giây, trạng thái sẽ báo "Vô hiệu" để phản ánh tình trạng thực tế 100%.
...
- **[core-engine/frontend]** Thêm các alias token bị thiếu (`text-muted`, `brand-primary`, `border-subtle`) vào `tailwind.config.ts` để sửa lỗi các component (`PluginCard`, `LaunchpadClient`, `Modal`, `Toast`) không render đúng màu sắc (Issue #257).
...
- **[core-engine/frontend]** Cập nhật `LaunchpadClient` sử dụng biến môi trường `NEXT_PUBLIC_*` thay vì hardcode localhost URLs, đảm bảo các link đến Mattermost, Outline Wiki, và n8n hoạt động đúng trên môi trường deploy qua Traefik (Issue #260).
